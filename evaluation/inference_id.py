...
def main():
    ...
    parser.add_argument("--output_path", type=str, default="../outputs/output_250730_v30.npy")
    ...
    # foreground인 것만 필터링
    fg_prompt_indices = (prompt_labels == 1).nonzero(as_tuple=False).squeeze()
    fg_prompt_labels = prompt_labels[fg_prompt_indices]
    fg_prompt_points = prompt_points[fg_prompt_indices]
    fg_instance_ids = instance_ids_np

    # 각 prompt point에 대해 생성된 binary mask를 저장할 리스트 shape
    raw_masks = [] 
    # 각 마스크에 대응하는 instance ID 저장 리스트
    raw_instance_ids = [] 

    with torch.no_grad():
        # 모든 prompt point를 순회하면서 하나씩 마스크 생성
        for i in range(fg_prompt_points.shape[0]):
            print(f"{i} 번째 prompt point 진행 중")
            
            single_point = fg_prompt_points[i].unsqueeze(0).unsqueeze(0) # 하나의 prompt point를 선택하고, 차원을 (1,1,3)으로 확장 -> Point-SAM의 입력 형태
            single_label = torch.tensor([[1]], dtype=torch.long).cuda()
            ...
            for j in range(after_mask3.shape[0]):
                ...
                raw_masks.append(m)
                raw_instance_ids.append(int(fg_instance_ids[i].item()))

    final_masks = apply_nms(raw_masks, raw_instance_ids, iou_thresh=args.nms_thresh)
